...
# real admin history request

from django.db.models.functions import TruncMonth
from django.db.models import Count, Sum
from django.shortcuts import render
from .models import BloodRequest  # بررسی کنید مدل شما همین نام را داشته باشد
from donor.models import BloodDonate  # بررسی کنید مدل شما همین نام را داشته باشد



def admin_request_history_view(request):
    # دریافت داده‌های ماهانه برای دریافت خون (فقط درخواست‌های تایید شده)
    received_blood = models.BloodRequest.objects.filter(
        Q(request_by_patient__isnull=False) | Q(request_by_donor__isnull=False),
        status='Approved'  # اضافه کردن فیلتر وضعیت تایید شده
    ).annotate(
    ...
    # دریافت داده‌های ماهانه برای اهدا خون (فقط اهداهای تایید شده)
    donated_blood = dmodels.BloodDonate.objects.filter(
        donor__isnull=False,
        status='Approved'  # اضافه کردن فیلتر وضعیت تایید شده
    ).annotate(
        month=TruncMonth('date')
        ...
        total_donated_units=Sum('unit')
    ).order_by('-month')

    return render(request, 'blood/admin_request_history.html', {
        'received_blood': received_blood,
...
def feedback(request):
    ...
    return render(request, 'blood/feedback.html', dic)
...
def contact_us(request):
    if request.method == 'POST':
        ...
        return render(request, 'blood/contact_us.html', {'success': True})
